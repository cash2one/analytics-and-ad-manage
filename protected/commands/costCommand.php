<?php
class costCommand extends CConsoleCommand
{
    ...
    private function _calculate($promoteArr,$server,$date)
    {
        ...
        {
            $detectedSql="SELECT id FROM data_daily where server_id={$server['id']} and channel_id={$channel} and date='{$date}'";
            $id = $dbLocal->createCommand($detectedSql)->queryScalar();
            if($id)
            {
...
/*                    $insertSql="INSERT INTO data_daily(`server_id`,`game_id`,`channel_id`,`open_time`,`cost`,`register`,`channel_register`
                  ...
                  ,'{$week}','{$month}')";*/
                    $updateSql = "update data_daily set `cost`='{$cost}',`cpa`='{$cpa}' where id={$id}";

...
class GameType extends CActiveRecord
{
	...
	{
	    $db = Yii::app()->db;
	    $dependency = new CDbCacheDependency('SELECT COUNT(id) FROM game_type');
	    $req = $db->cache(1000,$dependency)->createCommand("SELECT id,name FROM game_type");
	    $data = array();
	    $res = $req->queryAll();
	    if($res)
	    {
	...
	public static function getName($id)
	{
	    ...
	    {
	        $db = Yii::app()->db;
	        $req = $db->createCommand("SELECT name FROM game_type where id=:id LIMIT 1");
